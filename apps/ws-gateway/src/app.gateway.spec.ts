...
describe('WSGateway', () => {
  ...
  describe('registerExecutor', () => {
    it('should register executor and acknowledge', () => {
      ...
      const data = { executorId: 'executor-1', clientSocketId: 'client-1' };
  ...
  describe('pause', () => {
    it('should emit paused event to executor and acknowledge', () => {
      ...
      const response = gateway.pause(client);
      expect(response).toEqual({ status: 'ok' });
      expect(gateway.server.to('executor-1').emit).toHaveBeenCalledWith('paused');
    });
  });
  ...
  describe('resume', () => {
    it('should emit resumed event to executor and acknowledge', () => {
      const { client } = setupClientExecutorPair();
      const response = gateway.resume(client);
      expect(response).toEqual({ status: 'ok' });
      expect(gateway.server.to('executor-1').emit).toHaveBeenCalledWith('resumed');
    });
  });
  ...
  describe('cancel', () => {
    it('should emit canceled event to executor and acknowledge', () => {
      const { client } = setupClientExecutorPair();
      const response = gateway.cancel(client);
      expect(response).toEqual({ status: 'ok' });
      expect(gateway.server.to('executor-1').emit).toHaveBeenCalledWith('canceled');
    });
  });
  ...
  describe('writeTo', () => {
    it('should emit writeTo event to client and acknowledge', () => {
      ...
      const response = gateway.writeTo(executor, data);
      expect(response).toEqual({ status: 'ok' });
      expect(gateway.server.to('client-1').emit).toHaveBeenCalledWith('writeTo', data);
    });
  });
});
