...
import { ServerToClientEvents, ClientToServerEvents, AckResponse } from '@dataflow-ide/dataflow-core';
...
export class WSGateway implements OnGatewayDisconnect, OnGatewayConnection {
  ...
  private clientToExecutor = new Map<string, string>();
  private executorToClient = new Map<string, string>();

  handleConnection(client: Socket<ClientToServerEvents, ServerToClientEvents>, ...args: any[]) {
  ...
  writeTo(
    ...
    console.log("Received writeTo from executor", socket.id, "with data", data);
  ...
  handleDisconnect(client: Socket<ClientToServerEvents, ServerToClientEvents>) {
    ...
    if (clientSocketId) {
      ...
      console.log(`Cleaned up executor mapping for client ${clientSocketId}`);
    } else {
      ...
      if (executorSocketId) {
          ...
          console.log(`Cleaned up client mapping for executor ${executorSocketId}`);
      }
    }
  }
}
